...
Result: s4_k3_v2   (Chain-first GPU annealer (gpu_sc.py), corrected energy)
Complex: ∂Δ⁵ (S⁴)
This is exactly the definition used by the code that produced
cover.txt / planners.txt in this directory (`build("s4")` and the
...
all quintuples of {0..5}: itertools.combinations(range(6), 5):
...
The ordered (staircase) product lives on the 6 x 6 grid V x V.
Grid vertex (a, b) has id

      id(a, b) = 6*a + b

and the projections are pi1(id) = a, pi2(id) = b.  In cover.txt each
vertex of a facet of K x K is printed back as the pair (6*a+b -> (a,b)).
...
Total facets of K x K: 2520 = 6 x 6 pair(s) x 70 staircase path(s) per pair.
...
starting at pi1 and ending at pi2.  Each map is printed as a 6 x 6
table: row a, column b is the value (a vertex of K) of the map at the
grid vertex id = 6*a + b.  `@` marks grid vertices NOT in the domain
