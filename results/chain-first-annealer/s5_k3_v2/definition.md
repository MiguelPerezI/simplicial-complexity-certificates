...
Result: s5_k3_v2   (Chain-first GPU annealer (gpu_sc.py), corrected energy)
Complex: ∂Δ⁶ (S⁵)
...
all 6-tuples of {0..6}: itertools.combinations(range(7), 6):
...
K = ∂Δ⁶ is the boundary of the 6-simplex: these 7 facets are
exactly the facets of Δ6 (each omitting one vertex); K has f-vector
(7 21 35 35 21 7) and χ = 0, and is a closed
(4)-connected 5-manifold, hence homeomorphic to S⁵.
...
Total facets of K x K: 12348 = 7 x 7 pair(s) x 252 staircase
path(s) per pair = (n+1)^2 * C(2n-2, n-1) with n = 6.
