...
Result: s3_k3_v2   (Chain-first GPU annealer (gpu_sc.py), corrected energy)
Complex: ∂Δ⁴ (S³)
This is exactly the definition used by the code that produced
cover.txt / planners.txt in this directory (`build("s3")` and the
...
all quadruples of {0..4}: itertools.combinations(range(5), 4):
...
The ordered (staircase) product lives on the 5 x 5 grid V x V.
Grid vertex (a, b) has id

      id(a, b) = 5*a + b

and the projections are pi1(id) = a, pi2(id) = b.  In cover.txt each
vertex of a facet of K x K is printed back as the pair (5*a+b -> (a,b)).
...
Total facets of K x K: 500 = 5 x 5 pair(s) x 20 staircase path(s) per pair.
...
starting at pi1 and ending at pi2.  Each map is printed as a 5 x 5
table: row a, column b is the value (a vertex of K) of the map at the
grid vertex id = 5*a + b.  `@` marks grid vertices NOT in the domain
