...
Result: s6_k3_chain_first_continuation_01   (S6 guided continuation, chain-first annealer)
Complex: ∂Δ⁷ (S⁶)
This is exactly the definition used by the code that produced
cover.txt / planners.txt in this directory (`build("s6")` and the
...
all 7-tuples of {0..7}: itertools.combinations(range(8), 7):

  facet 0: [0, 1, 2, 3, 4, 5, 6]
  facet 1: [0, 1, 2, 3, 4, 5, 7]
  facet 2: [0, 1, 2, 3, 4, 6, 7]
  facet 3: [0, 1, 2, 3, 5, 6, 7]
  facet 4: [0, 1, 2, 4, 5, 6, 7]
  facet 5: [0, 1, 3, 4, 5, 6, 7]
  facet 6: [0, 2, 3, 4, 5, 6, 7]
  facet 7: [1, 2, 3, 4, 5, 6, 7]

K = ∂Δ⁷ is the boundary of the 7-simplex: these 8 facets are
exactly the facets of Δ⁷ (each omitting one vertex); K has f-vector
(8 28 56 70 56 28 8) and χ = 2, and is a closed
(5)-connected 6-manifold, hence homeomorphic to S⁶.
...
walks according to the mask, so it has p + q + 1 vertices (13 here).

Total facets of K x K: 59136 = 8 x 8 facet pair(s) x
924 staircase path(s) per pair = (n+1)^2 * C(2n-2, n-1) with n = 7.
